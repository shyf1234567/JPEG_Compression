...
{
    for (int i = 0; i < 8; i++)
        DCTMatrix[0][i] = 1 / (2 * SQRT2);
    for (int i = 1; i < 8; i++)
        for (int j = 0; j < 8; j++)
            DCTMatrix[i][j] = (0.5 * cos((2 * j + 1) * i / 16.0 * PI));
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
...
{
    vector <vector <float> > tmp(8,emptyFloat);
    ...
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
            tmp[i][j] = block[i][j];
    //DCT transfer
    tmp = Product(Product(DCTMatrix, tmp), DCTMatrixT);
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
...
vector <vector<int> > IDCT(vector <vector<int> > block)
{
    ...
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
            tmp[i][j] = block[i][j];
    tmp = Product(Product(DCTMatrixT, tmp), DCTMatrix);
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
            result[i][j] = tmp[i][j];
    return result;
